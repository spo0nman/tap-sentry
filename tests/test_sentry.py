...
class TapSentryTestCase(unittest.TestCase):
    ...
    def test_projects(self, m):
        ...
        m.get(
            "https://sentry.io/api/0//organizations/split-software/projects/",
            json=[record_value],
        )
    ...
    def test_sync_projects(self, m):
        ...
        record_value = load_file_current("projects_output.json", "data_test")
        with mock.patch(
            "tap_sentry.SentryClient.projects", return_value=[record_value]
        ):
    ...
    def test_events(self, m):
        ...
        m.get(
            "https://sentry.io/api/0//organizations/split-software/events/?project=1",
            json=[record_value],
        )
    ...
    def test_sync_events(self, m):
        ...
        with mock.patch.object(SentryClient, "projects", return_value=[{"id": 1}]):
            with mock.patch(
                "tap_sentry.SentryClient.events", return_value=[record_value]
            ):
                dataSync = SentrySync(self.client)
                schema = load_file("events.json", "tap_sentry/schemas")
                resp = dataSync.sync_events(
    ...
    def test_issues(self, m):
        ...
        m.get(
            "https://sentry.io/api/0//organizations/split-software/issues/?project=1",
            json=[record_value],
        )
    ...
    def test_sync_issues(self, m):
        ...
        dataSync.projects = [
            {"id": "1", "slug": "test-project", "organization": {"slug": "test-org"}}
        ]
    ...
    def test_teams(self, m):
        ...
        m.get(
            "https://sentry.io/api/0//organizations/split-software/teams/",
            json=[record_value],
        )
    ...
    def test_users(self, m):
        ...
        m.get(
            "https://sentry.io/api/0//organizations/split-software/users/",
            json=[record_value],
        )
    ...
    def test_sync_releases(self, m):
        ...
        with mock.patch.object(SentryClient, "projects", return_value=[{"id": 1}]):
            with mock.patch(
                "tap_sentry.SentryClient.releases", return_value=[record_value]
            ):
                dataSync = SentrySync(self.client)
    ...
    def test_full_sync_pipeline(self, mock_sync):
        ...
        from tap_sentry import get_selected_streams
    ...
    def test_custom_url_and_organization(self, m):
        ...
        custom_client = SentryClient(
            auth, url="https://de.sentry.io/api/0/", organization="custom-org"
        )
        ...
        issues_endpoint = (
            "https://de.sentry.io/api/0//organizations/custom-org/issues/?project=1"
        )
