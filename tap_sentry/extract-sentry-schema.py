...
def process_property(prop_name: str, prop_schema: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if prop_schema.get("type") == "object" and "properties" in prop_schema:
        ...
        for nested_name, nested_schema in prop_schema["properties"].items():
            singer_property["properties"][nested_name] = process_property(nested_name, nested_schema)
...
def find_schema_component(api_schema: Dict[str, Any], schema_ref: str) -> Dict[str, Any]:
...
def extract_path_schema(api_schema: Dict[str, Any], path_pattern: str, schema_ref: str) -> Dict[str, Any]:
    """Extract schema for specific path pattern and schema reference."""
    singer_schema = {
    ...
    if schema_ref:
        ...
        if component_schema and "properties" in component_schema:
            ...
            for prop_name, prop_schema in component_schema["properties"].items():
                singer_schema["properties"][prop_name] = process_property(prop_name, prop_schema)
                        ...
                        if "content" in response and "application/json" in response["content"]:
                            ...
                            if schema.get("type") == "array" and "items" in schema:
                                ...
                                if "properties" in item_schema:
                                    for prop_name, prop_schema in item_schema["properties"].items():
                                        singer_schema["properties"][prop_name] = process_property(prop_name, prop_schema)
...
def main():
    ...
    for arg in sys.argv[1:]:
        if not arg.startswith("--") and arg in [mapping["file"] for mapping in SCHEMA_MAPPING.values()]:
            specific_file = arg
    ...
    for path_pattern, mapping in SCHEMA_MAPPING.items():
        ...
        singer_schema = extract_path_schema(api_schema, path_pattern, schema_ref)
        ...
        if not preview_only:
            with open(output_path, 'w') as f:
                f.write(new_content)
            print(f"Saved schema to {output_path} with {len(singer_schema['properties'])} properties")
